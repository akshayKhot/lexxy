/**
 * <lexxy-editor> Custom Element
 *
 * The core editor component that wraps Meta's Lexical framework as a Web Component.
 *
 * ## Purpose
 *
 * This custom element provides a fully-featured rich text editor that acts as a native
 * form control. It handles the entire editing experience from initialization through
 * form submission.
 *
 * ## How It Works
 *
 * 1. **Form Integration**: Uses the ElementInternals API to participate in HTML forms as
 *    a first-class control. This means it:
 *    - Exposes a `value` property that contains the HTML content
 *    - Supports standard attributes like `name`, `required`, `disabled`
 *    - Participates in form validation
 *    - Responds to form reset events
 *
 * 2. **Lexical Editor Lifecycle**: Creates and manages a Lexical editor instance:
 *    - Registers required plugins (rich text, lists, markdown, code highlighting)
 *    - Defines custom nodes (attachments, custom attachables, horizontal dividers)
 *    - Sets up event listeners for changes, focus, blur
 *    - Handles bidirectional HTML ↔ Lexical conversion
 *
 * 3. **Component Coordination**: Manages sub-systems:
 *    - Contents: High-level content manipulation operations
 *    - Selection: Tracks and manages editor selection state
 *    - Clipboard: Handles paste events and special clipboard behaviors
 *    - Toolbar: Connects to formatting toolbar (if enabled)
 *
 * 4. **Value Synchronization**: On each editor update:
 *    - Converts Lexical's internal state to HTML
 *    - Sanitizes the HTML (removes unsafe content)
 *    - Updates the form value via ElementInternals
 *    - Fires "lexxy:change" event for external listeners
 *
 * 5. **Turbo Compatibility**: Handles Turbo caching by cleaning up editor state
 *    before cache snapshots and restoring on reconnection.
 *
 * ## Attributes
 *
 * - `name` (required): Form field name for submission
 * - `value`: Initial HTML content
 * - `placeholder`: Placeholder text when empty
 * - `required`: Makes the field required for validation
 * - `disabled`: Disables editing
 * - `toolbar`: "false" to disable, or ID of external toolbar element
 * - `attachments`: "false" to disable file attachments
 * - `single-line`: Restricts editor to single line (disables Enter key)
 * - `tabindex`: Tab order position
 *
 * ## Events
 *
 * - `lexxy:initialize`: Fired once when editor is ready
 * - `lexxy:change`: Fired when content changes
 * - `lexxy:focus`: Fired when editor gains focus
 * - `lexxy:blur`: Fired when editor loses focus
 * - `lexxy:file-accept`: Fired before file upload (cancellable)
 * - `lexxy:insert-link`: Fired when link is pasted (for unfurling)
 *
 * ## Usage
 *
 * Generated by Rails form helpers:
 *
 *     <%= form.rich_text_area :body %>
 *
 * Renders as:
 *
 *     <lexxy-editor name="post[body]" value="<p>Initial content</p>">
 *       <!-- Optional child elements like <lexxy-prompt> -->
 *     </lexxy-editor>
 *
 * @class LexicalEditorElement
 * @extends HTMLElement
 */
